...
description: Use when deploying Drupal sites to production environments - configuring Platform.sh (platform.app.yaml, routes.yaml, services.yaml, CLI commands), deploying on Pantheon (Terminus CLI, Drush aliases, multidev), deploying on Acquia Cloud (acli, deploy hooks, pipelines), implementing zero-downtime atomic deployments with symlinks, using drush deploy (updb + cim + cr in correct order), configuring maintenance mode during deployments, setting up environment-specific settings.php with getenv(), creating CI/CD pipelines for automated deployment (GitLab CI, GitHub Actions), running post-deploy verification, implementing rollback procedures, or managing database syncing between environments in Drupal 8-11+
---
...
Référentiel complet du déploiement Drupal 8-11+ : Platform.sh, Pantheon, Acquia, déploiement zéro-temps-d'arrêt, `drush deploy`, settings.php par environnement, CI/CD automatisé, et procédures de rollback. Couvre les 3 principales plateformes d'hébergement Drupal managé.

## 🎯 Choisir la Stratégie de Déploiement

```
Hébergement managé Drupal
  → Platform.sh : git push + pipeline automatique (le plus flexible)
  → Pantheon : Terminus + QuickSilver hooks (le plus simple)
  → Acquia : acli + pipelines (le plus enterprise)

Auto-hébergé (VPS, Docker, K8s)
  → Déploiement atomique avec symlinks (zéro-downtime)
...
| CLI Platform.sh | `platform ssh`, `platform db:dump` | [platform-sh.md](platform-sh.md) |
| Déployer sur Pantheon | `terminus env:deploy` | [acquia-pantheon.md](acquia-pantheon.md) |
| Sync DB Pantheon → local | `terminus remote:drush SITE.ENV -- sql:dump` | [acquia-pantheon.md](acquia-pantheon.md) |
| Déployer sur Acquia | `acli push:artifact` | [acquia-pantheon.md](acquia-pantheon.md) |
| Déploiement zéro-downtime (VPS) | Script atomic avec symlinks | [zero-downtime.md](zero-downtime.md) |
...
| CI/CD pipeline GitLab | `.gitlab-ci.yml` avec stages | [cicd-pipelines.md](cicd-pipelines.md) |
| CI/CD GitHub Actions | `.github/workflows/deploy.yml` | [cicd-pipelines.md](cicd-pipelines.md) |
